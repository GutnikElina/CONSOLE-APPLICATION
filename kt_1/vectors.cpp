...
std::string Vectors::FindUsername(std::vector<std::shared_ptr<User>> vector_user)
{
	...
	while (true)
	{
		try
		{
			...
			std::cout << "(áóêâû ëàòèíñêîãî àëôàâèòà, öèôğû, ïğîáåë)";
			Console::GoToXY(53, 15);
			username = CheckInputUsername();
			if ((int)username.size() < 5)
				throw "-!!!- ÏÎÏĞÎÁÓÉÒÅ ÇÀÍÎÂÎ. ËÎÃÈÍ ÄÎËÆÅÍ ÁÛÒÜ ÍÅ ÊÎĞÎ×Å 5 ÑÈÌÂÎËÎÂ -!!!-";
			else
			{
				for (int i = 0; i < (int)vector_user.size(); i++)
					if (username == vector_user.at(i)->GetUsername())
						throw "-!!!- ÏÎÏĞÎÁÓÉÒÅ ÇÀÍÎÂÎ. ÒÀÊÎÉ ÀÊÊÀÓÍÒ ÓÆÅ ÑÓÙÅÑÒÂÓÅÒ -!!!-";
			}
			return username;
		}
		catch (const char* m)
...
std::string Vectors::FindPassword()
{
	...
	while(true)
	{
		try
		{
			...
			std::cout << "(áóêâû ëàòèíñêîãî àëôàâèòà, öèôğû)";
			Console::GoToXY(54, 15);
			password = CheckInputPassword();
			std::string hashed_password = HashPassword(password);
			if (password.size() < 5)
				throw "-!!!- ÏÎÏĞÎÁÓÉÒÅ ÇÀÍÎÂÎ. ÏÀĞÎËÜ ÄÎËÆÅÍ ÁÛÒÜ ÍÅ ÊÎĞÎ×Å 5 ÑÈÌÂÎËÎÂ -!!!-";
...
void Vectors::GetVectorUsers()
{
	...
	file.clear();  // ñáğîñ ñîñòîÿíèÿ ïîòîêà äëÿ ïîâòîğíîãî èñïîëüçîâàíèÿ
	file.seekg(0);  // ïåğåõîä â íà÷àëî ôàéëà
		...
		for (int i = 0; i < vector_user.size(); i++)
			if (username == vector_user.at(i)->GetUsername() && password == vector_user.at(i)->GetPassword())
			{
				if (!vector_user.at(i)->GetAccess())
					throw "   -!!!- Ó×ÅÒÍÀß ÇÀÏÈÑÜ ÇÀÁËÎÊÈĞÎÂÀÍÀ -!!!-";
				else return vector_user.at(i)->GetUsername();
			}
		throw "-!!!- ÒÀÊÎÉ Ó×ÅÒÍÎÉ ÇÀÏÈÑÈ ÍÅ ÑÓÙÅÑÒÂÓÅÒ -!!!-";
	}
	catch (const char* m)
	{
		system("cls");
